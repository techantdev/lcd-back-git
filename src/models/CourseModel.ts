...
private courseLabel: String;
  ...
  toItem() {
    ...
    public static fromDB(item: CourseInterface) {
      ...
      // Partition keys
      newCourse.initializeKeys(newCourse.getPK(), newCourse.getSK());
  
      return newCourse.toItem();
    }
  
    public static async insertOne({ teacherId, yearGradeId, trackerId, courseLabel }: { teacherId: String; yearGradeId: String; trackerId: String; courseLabel: String }) {
      const newCourse = new Course();
